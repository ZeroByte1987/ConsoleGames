namespace ZConsole.Common
{
	...
	public class PropertyList : Dictionary<string, List<string>>
	{
		...
		public bool			GetBool(string propertyName)
		{
			...
			var boolString = this[propertyName][0].ToLower();
		...
		{
			return !ContainsKey(propertyName) ? "" : this[propertyName][0];
		}
		...
		{
			return !ContainsKey(propertyName) ? new List<string>() : this[propertyName];
		}
		...
		public  int			GetInt(string propertyName, int defaultValue)
		{
			...
			var intString = this[propertyName][0].ToLower();
	...
	public class ZConfig
	{
		...
		/// <returns>Returns resulting dictionary with pairs of categories and their appropriate values.</returns>
		public static PropertyList	ReadConfig(string source, Encoding encoding)
		{
			return ReadConfig(source, "[", "]", "", true, false, "\r\n", encoding);
		}
		...
		/// <param name="encoding">Encoding of config.</param>
		/// <returns>Returns resulting dictionary with pairs of categories and their appropriate values.</returns>
		public static PropertyList	ReadConfig(string source, string openingChars, string closingChars, string fillerChars, bool removeEmpty, bool isLowerCase, string newLineChars, Encoding encoding = null)
		{
			var resultProperptyList = new PropertyList();

			var sourceText = (File.Exists(source)) ? encoding == null ? ZFile.ReadTextFile(source) : ZFile.ReadTextFile(source, encoding) : source;
			...
			for (var i = 0; i < textLines.Length; i++)
			{
				...
				{
					var k = line.IndexOf(closingChars);
